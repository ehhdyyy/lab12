...
public class FamilyTree
{
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null)
			...
			throw new TreeException("Node with name " + name1 + " is not found");
		TreeNode node2 = root.getNodeWithName(name2);		// node whose name is name2
		if (node2 == null)
			//??? Throw TreeException with a useful message
			throw new TreeException("Node with name " + name2 + " is not found");
		
		// Get ancestors of node1 and node2.
		ArrayList<TreeNode> ancestorsOf1 = new ArrayList<>(node1.collectAncestorsToList());
		ArrayList<TreeNode> ancestorsOf2 = new ArrayList<>(node2.collectAncestorsToList());
